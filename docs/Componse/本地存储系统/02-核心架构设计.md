...
// 1. app.tsx - Root 组件
function Root() {
  const config = configManager.getConfig();  // 加载配置
...
export class ConfigManager {
  ...
  private stripEnvValues(config: ReasonCliConfig): ReasonCliConfig {
    // 将实际 API key 转换回环境变量引用
    // "sk-ant-xxx" → "${ANTHROPIC_API_KEY}"
  }
}
...
**关键设计：**
- **单例模式**: 全局唯一实例，避免重复加载
- **延迟加载**: 首次调用 `getConfig()` 时才加载
- **错误降级**: 任何错误都回退到默认配置
- **安全保存**: 自动移除敏感值

### 2. usePersistence Hook (hooks/usePersistence.ts)

**设计模式**: React Hooks + 函数式编程

**为什么不用中间件：**

```typescript
// ❌ 问题方案：Zustand 中间件自动保存
...
| 特性 | Zod | Valibot | 说明 |
|-----|-----|---------|------|
| `object()` | `z.object()` | `v.object()` | 对象定义 |
| `string()` | `z.string()` | `v.string()` | 字符串 |
| `number()` | `z.number()` | `v.number()` | 数字 |
| `optional()` | `z.optional()` | `v.optional()` | 可选字段 |
| `enum()` | `z.enum()` | `v.picklist()` | 枚举 |
| `record()` | `z.record()` | `v.record()` | 字典 |
| `pipe()` | `z.pipe()` | `v.pipe()` | 管道验证 |
| 类型推断 | `z.infer<>` | `v.InferOutput<>` | 类型提取 |
| 验证 | `parse()` | `parse()` | 同名 |
| 安全验证 | `safeParse()` | `safeParse()` | 同名 |
